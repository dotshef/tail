...
def cut_output_path(cuts_dir: Path, cut_num: int) -> Path:
    """컷 번호에 대한 출력 파일 경로."""
    return cuts_dir / f"컷_{cut_num:02d}.png"


def load_cut_data(
    prompts_json: Path, cut_num: int
) -> tuple[str, str, str, dict[str, str]]:
    """prompts.json에서 해당 cut의 (prompt, style_hint, aspect_ratio, characters)를 조회한다."""
    ...
    for cut_entry in data.get("cuts", []):
        if cut_entry.get("cut") == cut_num:
            return cut_entry["prompt"], style_hint, aspect_ratio, characters
...
def build_final_prompt(
    ...
    """컷 프롬프트에 캐릭터 Identity Header, 스타일 힌트, 비율을 합쳐 최종 프롬프트를 만든다."""
    parts = [
        "레퍼런스 이미지의 캐릭터와 아트 스타일을 정확히 유지하여 다음 장면을 그려줘.",
    ]
    if characters:
        parts += ["", "캐릭터 정보 (모든 컷에서 동일한 외형을 유지할 것):"]
        for name, desc in characters.items():
            parts.append(f"- {name}: {desc}")
    parts += ["", f"장면: {cut_prompt}"]
    if style_hint:
...
def main() -> int:
    ...
    # 이미 존재하는 컷은 스킵 (API 호출 전에 확인해 비용 절약)
    out_path = cut_output_path(cuts_dir, args.cut)
    if out_path.exists():
        print(f"[{args.story}] 컷 {args.cut}: 이미 존재하여 스킵 ({out_path})")
        return 0
    ...
    try:
        cut_prompt, style_hint, aspect_ratio, characters = load_cut_data(
            prompts_path, args.cut
        )
    ...
    final_prompt = build_final_prompt(cut_prompt, style_hint, aspect_ratio, characters)
