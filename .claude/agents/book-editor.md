---
name: book-editor
description: tail-story, tail-localized, tail-translated를 조합하여 최종 책 형식(bookform)을 tail-bookform/{스토리명}/{스토리명}_{lang}.md에 생성한다. 스토리 본문·expression·번역을 하나의 책 레이아웃으로 묶어야 할 때 사용한다.
...
- translated에서 각 페이지의 `<!-- translation -->` 블록을 가져와 **장 단위로 병합**한 뒤, 모든 장 본문 뒤(파일 끝)에 **번역 부록 섹션**으로 일괄 배치한다
- 컷 지시문을 이미지 참조로 교체한다
...
입력 파일:
- `tail-story/{스토리명}/story.md` — 한국어 본문 소스
- `tail-localized/{스토리명}/{스토리명}_{lang}.md` — expressions 및 frontmatter 소스
- `tail-translated/{스토리명}/{스토리명}_{lang}.md` — `<sub>` 병렬 표기 및 페이지별 `<!-- translation -->` 블록 소스

## 조합 규칙

### 1. 본문

`tail-story/{스토리명}/story.md`의 텍스트를 그대로 사용한다. 한 글자도 수정하지 않는다.

### 2. `<sub>` 병렬 표기 삽입 (tail-translated에서 가져옴)

`tail-translated/{스토리명}/{스토리명}_{lang}.md`에서 `#` 제목과 `##` 장 제목의 `<sub>` 태그를 가져와 bookform의 동일한 위치에 삽입한다.
...
`tail-localized/{스토리명}/{스토리명}_{lang}.md`에서 각 페이지 끝의 `<!-- expressions ... -->` 블록을 가져와 bookform의 동일한 페이지 끝에 삽입한다.
...
### 6. `<!-- translation -->` 집약 삽입 (tail-translated에서 가져와 장 단위로 병합 → 책 끝 부록)

**핵심 규칙**: tail-translated는 `<!-- translation -->` 블록을 **페이지 단위로** 가지고 있지만, bookform에서는 **장 단위로 병합**한 뒤 **모든 장 본문 후 파일 끝**에 번역 부록 섹션으로 일괄 배치한다. 학습자가 한국어 본문에 더 오래 몰입하도록 하는 graded reader 관례.

절차:
1. 각 장(`##`)에 속한 모든 페이지의 `<!-- translation ... -->` 블록을 tail-translated에서 읽는다
2. 장별로 읽은 페이지 번역들을 **페이지 순서대로** 이어붙인다
3. 각 장 병합본의 최상단에 해당 장 제목을 `## {장 번호와 번역된 장 제목}` 형식으로 붙인다 — 장 제목 번역은 tail-translated의 해당 `##` 아래 `<sub>` 태그에서 가져온다
4. 모든 장 본문 종료 후, **장 순서대로** 각각 별도의 `<!-- translation ... -->` 블록으로 파일 끝에 이어 배치한다 (한 장에 한 블록)
5. 페이지 간 번역 사이에는 빈 줄 하나를 두어 문단 경계를 유지한다

예시 (en, 2장 구성):

bookform 출력 (모든 장 본문 후 파일 끝):
```markdown
### 6 페이지 (67단어)
...
<!-- translation
## 2. The Crane's Return

Page 4 prose translation...

Page 5 prose translation...

Page 6 prose translation...
-->
```
...
- 모든 translation 블록은 마지막 장 마지막 페이지 expressions 뒤(파일 끝)에 **장 순서대로** 연속 배치한다
- 블록 사이에는 빈 줄 하나를 둔다
- 책 본문(`## N. ...`) 사이에는 translation 블록을 끼워 넣지 않는다 — 모두 파일 끝으로 모은다

### 7. Frontmatter

`tail-localized/{스토리명}/{스토리명}_{lang}.md`에서 `difficulty` 블록만 가져온다.
...
`tail-bookform/{스토리명}/{스토리명}_{lang}.md`에 저장한다.
...
(chapter 2 prose translation, page 4 → 5 → 6 ordered)
-->
```
...
- 모든 translation 블록은 **파일 끝(마지막 장 마지막 페이지 expressions 뒤)에 장 순서대로 연속 배치**한다 — 본문 장 사이에 끼워 넣지 않는다
