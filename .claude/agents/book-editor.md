...
- midform의 `<!-- translation -->` 블록을 가져와 각 장 끝에 끼워넣는다
- 컷 헤딩을 이미지 참조로 교체한다
- 새로운 문장을 창작하지 않는다
...
midform에서 각 장(`##`) 끝의 `<!-- translation ... -->` 블록을 가져와 bookform의 동일한 위치(해당 장 마지막 페이지의 expressions 블록 다음, 다음 `##` 직전)에 삽입한다.

- translation 블록은 해당 장 마지막 페이지의 expressions 블록 뒤에 빈 줄 하나를 두고 삽입한다
- midform의 translation 내용을 그대로 복사한다
- translation 블록과 다음 `##` 사이에도 빈 줄 하나를 둔다
