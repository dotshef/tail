...
- 프롬프트 작성은 하지 않는다 (prompt-writer 에이전트의 역할)
- 마크다운을 파싱하지 않는다 (prompt-writer가 이미 처리)
...
1. `temp-image/{스토리명}/prompts.json` — prompt-writer가 생성한 프롬프트 파일
...
2. `temp-image/{스토리명}/prompts.json` 존재를 확인한다
   - 없으면 에러 보고 후 중단 ("prompt-writer를 먼저 실행하세요")
