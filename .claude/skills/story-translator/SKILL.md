...
description: tail-story의 한국어 본문을 각국 언어(영어, 일본어)로 번역하여 tail-translated/{스토리명} 폴더에 저장하는 진입점
---
...
- 영어: `tail-translated/{스토리명}/{스토리명}_en.md`
- 일본어: `tail-translated/{스토리명}/{스토리명}_jp.md`

## 동작

1. 사용자의 요청에서 대상 스토리명을 파악한다
2. `tail-story/{스토리명}/story.md`가 존재하는지 확인한다 (없으면 에러 보고)
3. 출력 폴더(`tail-translated/{스토리명}/`)가 없으면 생성한다
4. **언어별로 story-translator 에이전트를 병렬 spawn한다** (2개 동시)
   ...
   - 출력: `tail-translated/{스토리명}/{스토리명}_{lang}.md`
