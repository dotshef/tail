---
name: localizer
description: tail-analyzed의 분석본을 각국 언어(영어, 중국어 번체, 일본어)로 로컬라이즈하여 tail-{lang} 폴더에 저장하는 진입점
---
...
3. 출력 폴더(`tail-en/`, `tail-zh-tw/`, `tail-ja/`)가 없으면 생성한다
4. **언어별로 localizer 에이전트를 병렬 spawn한다** (3개 동시)
   - 각 에이전트에 스토리명과 대상 언어를 전달한다
   - 동작: 분석본을 읽고, 제목/장 병렬 표기 + expression 번역
   - 출력: `tail-{lang}/{스토리명}.md`
