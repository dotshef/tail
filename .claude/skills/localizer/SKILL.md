---
name: localizer
description: tail-analyzed의 분석본을 각국 언어(영어, 일본어)로 로컬라이즈하여 tail-localized/{스토리명} 폴더에 저장하는 진입점
---
...
- 영어: `tail-localized/{스토리명}/{스토리명}_en.md`
- 일본어: `tail-localized/{스토리명}/{스토리명}_jp.md`

## 동작

1. 사용자의 요청에서 대상 스토리명을 파악한다
2. `tail-analyzed/{스토리명}/analysis.md`가 존재하는지 확인한다 (없으면 에러 보고)
3. 출력 폴더(`tail-localized/{스토리명}/`)가 없으면 생성한다
4. **언어별로 localizer 에이전트를 병렬 spawn한다** (2개 동시)
   - 각 에이전트에 스토리명과 대상 언어를 전달한다
   - 동작: 분석본을 읽고, `<!-- expressions -->` 뜻풀이를 대상 언어로 번역하고 frontmatter의 `reasoning` 필드 제거 + `lang` 필드 추가
   - 출력: `tail-localized/{스토리명}/{스토리명}_{lang}.md`
