---
name: book-editor
description: tail-story, tail-localized, tail-translated를 조합하여 tail-bookform/{스토리명}에 최종 책 형식을 생성하는 진입점
---
...
- 스토리 본문: `tail-story/{스토리명}/story.md`
- Expression/frontmatter 소스: `tail-localized/{스토리명}/{스토리명}_{lang}.md`
- 번역 소스 (`<sub>` + 페이지별 `<!-- translation -->`): `tail-translated/{스토리명}/{스토리명}_{lang}.md`
- 이미지: `tail-image/{스토리명}/cuts/컷{N}.jpeg`

### 출력

- `tail-bookform/{스토리명}/{스토리명}_{lang}.md` (localized와 translated 모두 존재하는 언어별로 생성)

## 동작

1. 사용자의 요청에서 대상 스토리명을 파악한다
2. `tail-story/{스토리명}/story.md`가 존재하는지 확인한다 (없으면 에러 보고)
3. `tail-localized/{스토리명}/` 및 `tail-translated/{스토리명}/`을 탐색하여 해당 스토리가 **양쪽 모두에 존재하는 언어**(파일명 suffix `_{lang}`)를 확인한다
   - 두 곳 중 한쪽에만 있는 언어는 건너뛰고 경고한다 (bookform 조합에 두 소스가 모두 필요하기 때문)
   - 어떤 언어에서도 양쪽이 다 갖춰지지 않았다면 에러 보고
4. `tail-bookform/{스토리명}/` 폴더가 없으면 생성한다
5. **양쪽에 존재하는 언어별로 book-editor 에이전트를 병렬 spawn한다**
   ...
   - 동작:
     ...
     - tail-translated의 페이지별 `<!-- translation -->` 블록을 **장 단위로 병합**하여 모든 장 본문 뒤(파일 끝)에 부록으로 일괄 배치
     - tail-localized의 frontmatter `difficulty` 블록만 가져오기
   - 출력: `tail-bookform/{스토리명}/{스토리명}_{lang}.md`
